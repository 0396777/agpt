...
class OpenLLAMAPEFTModel(nn.Module):
    def __init__(self, **args):
        ...
        # 分阶段加载
        self.init_vision_encoder(imagebind_ckpt_path)
        self.init_language_decoder(vicuna_ckpt_path)
        self.init_auxiliary_modules()
    ...
    def init_vision_encoder(self, ckpt_path):
        ...
        # 直接加载权重到 GPU
        imagebind_ckpt = torch.load(ckpt_path, map_location='cuda')
    ...
    def init_language_decoder(self, ckpt_path):
        """分阶段加载语言解码器"""
        print(f'Initializing language decoder from {ckpt_path} ...')
        # 初始化语言模型结构
        self.llama_model = LlamaForCausalLM.from_pretrained(ckpt_path)
        # 配置 LoRA
        ...
        # 初始化 Tokenizer
        self.llama_tokenizer = LlamaTokenizer.from_pretrained(ckpt_path, use_fast=False)
        ...
        if 'masks' in inputs:
            ...
            if random.randint(0,1) == 0 and len(inputs['img_paths']) == len(image_paths):
                ...
                for path in inputs['img_paths']:
                    if 'all_anomalygpt' not in path and 'visa-data' in path.lower():
