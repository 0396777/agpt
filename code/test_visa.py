...
args = {
    ...
    'anomalygpt_ckpt_path': '/kaggle/input/train-supervised-m/pytorch_model.pt',
...
# -------------------------------
# 阶段1: 加载视觉编码器
model.init_vision_encoder(args['imagebind_ckpt_path'])

# 阶段2: 加载语言解码器
model.init_language_decoder(args['vicuna_ckpt_path'])

# 阶段3: 加载其他模块和检查点
model.init_auxiliary_modules()

# 加载Delta和AnomalyGPT权重（直接到GPU）
print("加载Delta检查点...")
...
# 最终设置为评估模式
model = model.eval()
print(f"最终GPU显存占用: {torch.cuda.memory_allocated()/1024**3:.2f} GB")
